#include "CXMLSwitchProbabilistNodeReader.hpp"
...
EParsingStatus CXMLSwitchProbabilistNodeReader::openChild(const char* sName, const char** sAttributeName, const char** sAttributeValue, uint64 ui64AttributeCount, IAutomatonContext* pContext)
{
	...
	else
	{
...
#ifdef DEBUG
		cout<<"SwitchProbabilistReader : Opening node : "<<l_oChildName<<endl;
#endif
		...
		else if(l_oChildName == "Property" && m_eStatus==ParsingStatus_Child)
		{
			...
			{
				cout<<"Missing attribute class(=Probability) in Property node"<<endl;
				m_eStatus = ParsingStatus_Error;
			}
		}
...
EParsingStatus CXMLSwitchProbabilistNodeReader::processChildData(const char* sData, IAutomatonContext* pContext)
{
	//If we have to redirect to another reader
	if(m_oReaderStack.size() != 0)
	{
...
#ifdef DEBUG
		cout<<"SwitchProbabilistReader : Processing node : "<<l_oChildName<<"..."<<endl;
#endif
...
EParsingStatus CXMLSwitchProbabilistNodeReader::closeChild(IAutomatonContext* pContext)
{
	//If we have to redirect to another reader
	if(m_oReaderStack.size() != 0)
	{
...
void CXMLSwitchProbabilistNodeReader::release()
{
	delete this;
}
