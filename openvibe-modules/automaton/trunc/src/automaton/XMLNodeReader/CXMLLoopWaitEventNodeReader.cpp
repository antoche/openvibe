#include "CXMLLoopWaitEventNodeReader.hpp"
...
CXMLLoopWaitEventNodeReader::CXMLLoopWaitEventNodeReader() :
...
EParsingStatus CXMLLoopWaitEventNodeReader::openChild(const char* sName, const char** sAttributeName, const char** sAttributeValue, uint64 ui64AttributeCount, IAutomatonContext* pContext)
{
...
#ifdef DEBUG
		cout<<"LoopWaitEventReader : Opening node : "<<l_oChildName<<endl;
#endif
...
EParsingStatus CXMLLoopWaitEventNodeReader::processChildData(const char* sData, IAutomatonContext* pContext)
{
	//If we have to redirect to another reader
	if(m_oReaderStack.size() != 0)
	{
...
#ifdef DEBUG
		cout<<"LoopWaitEventReader : Processing node : "<<l_oChildName<<"..."<<endl;
#endif
...
EParsingStatus CXMLLoopWaitEventNodeReader::closeChild(IAutomatonContext* pContext)
{
	//If we have to redirect to another reader
	if(m_oReaderStack.size() != 0)
	{
	...
	else
	{
...
#ifdef DEBUG
		cout<<"LoopWaitEventReader : Closing node : "<<l_oChildName<<endl;
#endif
		...
		{
			m_pNode->setLoopCondition("WaitEvent", m_oParameter, pContext);
...
void CXMLLoopWaitEventNodeReader::release()
{
	delete this;
}
