...
boolean CNaiveBayesApplyBoxAlgorithm::initialize(void)
{
	...
	m_pStreamEncoder.resize(getStaticBoxContext().getOutputCount());
	for(uint32 i_out=0; i_out<l_rStaticBoxContext.getOutputCount(); i_out++)
	{
		m_pStreamEncoder[i_out]=&getAlgorithmManager().getAlgorithm(getAlgorithmManager().createAlgorithm(OVP_GD_ClassId_Algorithm_StreamedMatrixStreamEncoder));
		m_pStreamEncoder[i_out]->initialize();
	    }
	...
	m_pStreamEncoder[0]->getInputParameter(OVP_GD_Algorithm_StreamedMatrixStreamEncoder_InputParameterId_Matrix)->setValue(&l_oEncodeDecisionMatrix);
	
	
	m_oEncodeClassificationProbabilitiesMatrix.setDimensionCount(2);
	m_oEncodeClassificationProbabilitiesMatrix.setDimensionSize(0, m_ui64NbEntries);
	m_oEncodeClassificationProbabilitiesMatrix.setDimensionSize(1, m_ui32NbClasses);
	
	
			//this->getLogManager() << LogLevel_Info << "m_ui32NbClasses*m_ui64NbEntries: " << m_ui32NbClasses*m_ui64NbEntries << " loaded\n";

	IMatrix *l_oEncodeClassificationProbabilitiesMatrix=&m_oEncodeClassificationProbabilitiesMatrix;
	m_pStreamEncoder[1]->getInputParameter(OVP_GD_Algorithm_StreamedMatrixStreamEncoder_InputParameterId_Matrix)->setValue(&l_oEncodeClassificationProbabilitiesMatrix);
...
boolean CNaiveBayesApplyBoxAlgorithm::uninitialize(void)
{
	...
	for(uint32 i_out=0; i_out<l_rStaticBoxContext.getOutputCount(); i_out++)
	{
	      m_pStreamEncoder[i_out]->uninitialize();
	      getAlgorithmManager().releaseAlgorithm(*m_pStreamEncoder[i_out]);
	}
	m_pStreamEncoder.clear();
...
boolean CNaiveBayesApplyBoxAlgorithm::process(void)
{
	...
	{
		// Decision
		float64 *l_pf64EncodeMatrix, *l_pf64EncodeProbabilitiesMatrix, l_f64MaxAbsFunctional=0.0, l_f64SumExpFunctional1and2, l_f64DiffMin;
		// uint32 l_ui32IndMin;

		l_pf64EncodeMatrix = m_oEncodeDecisionMatrix.getBuffer();
		l_pf64EncodeProbabilitiesMatrix = m_oEncodeClassificationProbabilitiesMatrix.getBuffer();
		...
		for (uint32 i_ent=0; i_ent<m_ui64NbEntries; i_ent++)
		{
			...
			m_pf64AllEntriesClassFunctionalNB[i_ent][0] = m_pf64AllEntriesClassFunctionalNB[i_ent][0] / l_f64SumExpFunctional1and2;
			m_pf64AllEntriesClassFunctionalNB[i_ent][1] = m_pf64AllEntriesClassFunctionalNB[i_ent][1] / l_f64SumExpFunctional1and2;
			l_pf64EncodeProbabilitiesMatrix[i_ent]=(float64)m_pf64AllEntriesClassFunctionalNB[i_ent][0];
			l_pf64EncodeProbabilitiesMatrix[i_ent+m_ui64NbEntries]=(float64)m_pf64AllEntriesClassFunctionalNB[i_ent][1];
		...
		l_pf64EncodeMatrix[0] = (float64)l_ui32Index;
		//this->getLogManager()<<LogLevel_Info<<"NB entries: "<<l_pf64EncodeMatrix[0]<<"\n";
		//this->getLogManager()<<LogLevel_Info<<"NB proba[0]: "<<l_pf64EncodeProbabilitiesMatrix[0]<<"\n";
		//this->getLogManager()<<LogLevel_Info<<"NB proba[1]: "<<l_pf64EncodeProbabilitiesMatrix[11]<<"\n";
		...
		if(!m_bHasSentHeader)
		{
		    for(uint32 i_out=0; i_out<l_rStaticBoxContext.getOutputCount(); i_out++)
		    {
			TParameterHandler < IMemoryBuffer* > l_oOutputMemoryBufferHandle(m_pStreamEncoder[i_out]->getOutputParameter(OVP_GD_Algorithm_StreamedMatrixStreamEncoder_OutputParameterId_EncodedMemoryBuffer));
			l_oOutputMemoryBufferHandle=l_rDynamicBoxContext.getOutputChunk(i_out);
			m_pStreamEncoder[i_out]->process(OVP_GD_Algorithm_StreamedMatrixStreamEncoder_InputTriggerId_EncodeHeader);
			l_rDynamicBoxContext.markOutputAsReadyToSend(i_out, m_ui64LastEndTime, m_ui64LastEndTime);	
		    }
		...
		{	
			TParameterHandler < IMemoryBuffer* > l_oOutputMemoryBufferHandle(m_pStreamEncoder[i_out]->getOutputParameter(OVP_GD_Algorithm_StreamedMatrixStreamEncoder_OutputParameterId_EncodedMemoryBuffer));
			l_oOutputMemoryBufferHandle=l_rDynamicBoxContext.getOutputChunk(i_out);
			m_pStreamEncoder[i_out]->process(OVP_GD_Algorithm_StreamedMatrixStreamEncoder_InputTriggerId_EncodeBuffer);
			l_rDynamicBoxContext.markOutputAsReadyToSend(i_out, m_ui64LastStartTime, l_ui64ReadChunkMinEndTime);
		}
