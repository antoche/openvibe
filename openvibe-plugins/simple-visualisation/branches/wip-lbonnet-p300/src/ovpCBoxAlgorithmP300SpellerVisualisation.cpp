...
namespace
{
	...
	static void redraw_cb(GtkWidget *pWidget, GdkEventExpose *pEvent, gpointer pUserData)
	{
		//printf("entering callback...\n");

		reinterpret_cast<CBoxAlgorithmP300SpellerVisualisation*>(pUserData)->redraw();
		//return FALSE;
	}
};

void CBoxAlgorithmP300SpellerVisualisation::redraw()
{
	//m_bFlashTriggered = true;
	
	if(!m_bFlashTriggered)
	{
		uint64 l_ui64FlashTime = m_ui64LastPlayerTime + (System::Time::zgetTime() - m_ui64LastSystemTime);

		m_pTargetStimulationEncoder->getInputStimulationSet()->clear();
		m_pResyncFlashSequenceStimulationEncoder->getInputStimulationSet()->clear();
		
		if(m_ui64LastFlashIdentifier == m_ui64LastColumnTargetIdentifier || m_ui64LastFlashIdentifier == m_ui64LastRowTargetIdentifier)
		{
			m_pTargetStimulationEncoder->getInputStimulationSet()->appendStimulation(OVTK_StimulationId_Target, l_ui64FlashTime, 0);
		}
		else
		{
			m_pTargetStimulationEncoder->getInputStimulationSet()->appendStimulation(OVTK_StimulationId_NonTarget, l_ui64FlashTime, 0);
		}

		m_pResyncFlashSequenceStimulationEncoder->getInputStimulationSet()->appendStimulation(m_ui64LastFlashIdentifier, l_ui64FlashTime, 0);

		m_pTargetStimulationEncoder->encodeBuffer(1);
		m_pResyncFlashSequenceStimulationEncoder->encodeBuffer(0);

		m_bFlashTriggered = true;
		
		this->getDynamicBoxContext().markOutputAsReadyToSend(0, m_ui64LastOutputChunkDate, l_ui64FlashTime);
		m_ui64LastOutputChunkDate = l_ui64FlashTime;

		this->getLogManager() << LogLevel_Debug << "timings are : Flash[" << time64(l_ui64FlashTime) << "] Player[" << time64(this->getPlayerContext().getCurrentTime())<<"]\n";
	}
...
boolean CBoxAlgorithmP300SpellerVisualisation::initialize(void)
{
	...
	m_pTargetStimulationEncoder = new TStimulationEncoder<CBoxAlgorithmP300SpellerVisualisation>(*this);
	m_pResyncFlashSequenceStimulationEncoder = new TStimulationEncoder<CBoxAlgorithmP300SpellerVisualisation>(*this);
	m_pTargetStimulationEncoder->getInputStimulationSet().setReferenceTarget(op_pSequenceStimulationSet);
	m_pResyncFlashSequenceStimulationEncoder->getInputStimulationSet().setReferenceTarget(op_pSequenceStimulationSet);
	...
	//if it does not already exist we buildup our own signal
	if(g_signal_lookup("flash",G_TYPE_OBJECT) == 0)
	{
		g_signal_new("flash",
	...
	for(uint32 i = 0; i < l_uiRowCount; i++)
	{
		for(uint32 j = 0; j < l_uiRowCount; j++)
		{
			g_signal_connect(G_OBJECT(m_vCache[i][j].pWidget), "flash", G_CALLBACK(redraw_cb), this);
		}
	}
	...
	m_bFlashTriggered = false;
	m_ui64LastFlashIdentifier = (uint64) -1;
	m_ui64LastColumnTargetIdentifier = (uint64) -1;
	m_ui64LastRowTargetIdentifier = (uint64) -1;
	m_ui64LastOutputChunkDate = 0;
	m_ui64LastPlayerTime = 0;
	m_ui64LastSystemTime= 0;
...
boolean CBoxAlgorithmP300SpellerVisualisation::uninitialize(void)
{
	...
	if(m_pTargetStimulationEncoder)
	{
		m_pTargetStimulationEncoder->uninitialize();
		delete m_pTargetStimulationEncoder;
	}

	if(m_pResyncFlashSequenceStimulationEncoder)
	{
		m_pResyncFlashSequenceStimulationEncoder->uninitialize();
		delete m_pResyncFlashSequenceStimulationEncoder;
	}
...
boolean CBoxAlgorithmP300SpellerVisualisation::processClock(CMessageClock& rMessageClock)
{
	...
	if(m_bFlashTriggered)
	{
		...
		if(m_ui64LastFlashIdentifier == m_ui64LastColumnTargetIdentifier || m_ui64LastFlashIdentifier == m_ui64LastRowTargetIdentifier)
		{
			m_pTargetStimulationEncoder->getInputStimulationSet()->appendStimulation(OVTK_StimulationId_Target, this->getPlayerContext().getCurrentTime(), 0);
		}
		else
		{
			m_pTargetStimulationEncoder->getInputStimulationSet()->appendStimulation(OVTK_StimulationId_NonTarget, this->getPlayerContext().getCurrentTime(), 0);
		}

		m_pResyncFlashSequenceStimulationEncoder->getInputStimulationSet()->appendStimulation(m_ui64LastFlashIdentifier, this->getPlayerContext().getCurrentTime(), 0);
	...
	this->getDynamicBoxContext().markOutputAsReadyToSend(0, m_ui64LastOutputChunkDate, this->getPlayerContext().getCurrentTime());
	m_ui64LastOutputChunkDate = this->getPlayerContext().getCurrentTime();*/
	m_ui64LastPlayerTime = this->getPlayerContext().getCurrentTime();
	m_ui64LastSystemTime= System::Time::zgetTime();
	return true;
}
...
boolean CBoxAlgorithmP300SpellerVisualisation::process(void)
{
	...
	for(i=0; i<l_rDynamicBoxContext.getInputChunkCount(0); i++)
	{
		...
		{
			m_pResyncFlashSequenceStimulationEncoder->encodeHeader(0);
			m_pTargetStimulationEncoder->encodeHeader(1);
		}
		...
		if(m_pSequenceStimulationDecoder->isOutputTriggerActive(OVP_GD_Algorithm_StimulationStreamDecoder_OutputTriggerId_ReceivedBuffer))
		{
			...
			for(j=0; j<l_pStimulationSet->getStimulationCount(); j++)
			{
				...
				if(l_ui64StimulationIdentifier >= m_ui64RowStimulationBase && l_ui64StimulationIdentifier < m_ui64RowStimulationBase+m_ui64RowCount)
				{
					...
					l_bFlash=true;
					m_bFlashTriggered = false;
					m_ui64LastFlashIdentifier = l_ui64StimulationIdentifier;
				...
				if(l_bFlash)
				{
					...
					this->_cache_for_each_if_(
						l_iRow,
						l_iColumn,
						&CBoxAlgorithmP300SpellerVisualisation::_cache_emit_signal_cb_,
						&CBoxAlgorithmP300SpellerVisualisation::_cache_emit_signal_cb_,
						NULL,
						NULL);
				}
		...
		{
			m_pResyncFlashSequenceStimulationEncoder->encodeEnd(0);
			m_pTargetStimulationEncoder->encodeEnd(1);
		}
			...
			if(m_pTargetStimulationDecoder->isOutputTriggerActive(OVP_GD_Algorithm_StimulationStreamDecoder_OutputTriggerId_ReceivedBuffer))
			{
				...
				for(j=0; j<l_pStimulationSet->getStimulationCount(); j++)
				{
					...
					if(l_ui64StimulationIdentifier >= m_ui64RowStimulationBase && l_ui64StimulationIdentifier < m_ui64RowStimulationBase+m_ui64RowCount)
					{
						...
						l_bTarget=true;
						m_ui64LastRowTargetIdentifier = l_ui64StimulationIdentifier;
					}
					if(l_ui64StimulationIdentifier >= m_ui64ColumnStimulationBase && l_ui64StimulationIdentifier < m_ui64ColumnStimulationBase+m_ui64ColumnCount)
					{
						...
						l_bTarget=true;
						m_ui64LastColumnTargetIdentifier = l_ui64StimulationIdentifier;
					}
...
void CBoxAlgorithmP300SpellerVisualisation::_cache_emit_signal_cb_(CBoxAlgorithmP300SpellerVisualisation::SWidgetStyle& rWidgetStyle, void* pUserData)
{
	g_signal_emit_by_name(rWidgetStyle.pWidget, "flash");
}
